...
//					System.out.println(chemicalName+"\t"+reference);
	...
	private static String getValue(String headerLine, String line, String field,int extraChars) {
		...
		if(line.length()>headerLine.indexOf(field)) {
			...
			String newValue = extractFieldValue(headerLine, line, field);
			...
			if(countRemoved>3) {
				// Use the new value generated by extractFieldValue to avoid excessive spaces
				// extractFieldValue has been checked to work for all cases where countRemoved > 3
				// but still can return an incorrect value for some cases where countRemoved <= 3, so we only use it when countRemoved > 3

				// Logging the change for debugging purposes
				// if (newValue != null) {
				// 	System.out.println("Removed "+countRemoved+" spaces for field="+field+"\n"+headerLine+"\n"+line+"\nValue = "+value+"\nNew Value = "+newValue+"\n");
				// }
...
//			value=clean(value);
			...
			// Logging the difference between the original value and the new value generated by extractFieldValue for debugging purposes
			// Will attempt to harmonize values from getValue and extractFieldValue in the future to enable only using extractFieldValue for all cases
			// if (!value.equals(newValue)) {
			// 	System.out.println("Value mismatch for field="+field+"\n"+headerLine+"\n"+line+"\nValue = "+value+"\nNew Value = "+newValue+"\n");
			// }
	...
	public static String extractFieldValue(String headerLine, String dataLine, String fieldName) {
        if (headerLine == null || dataLine == null || fieldName == null) {
            return null;
        }

		Pattern numericValue = Pattern.compile("^[<>]?-?\\d+(?:\\.\\d+)?(?:[eE][+-]?\\d+)?(?:\\(.*\\))?\\*?$");
		// Pattern numericValue = Pattern.compile("^[<>]?-?\\\\d+(?:,?\\\\d{3})*(?:\\\\.\\\\d+)?(?:[eE][+-]?\\\\d+)?(?:-\\\\d+(?:\\\\.\\\\d+)?)?(?:\\\\([^)]*\\\\))?\\\\*?$");
		Boolean isNumericField = fieldName.equalsIgnoreCase("Kd") || fieldName.equalsIgnoreCase("Koc") || fieldName.equalsIgnoreCase("%om") || fieldName.equalsIgnoreCase("pH") || fieldName.equalsIgnoreCase("temp");
        ...
        while (i < headerLine.length()) {
            while (i < headerLine.length() && Character.isWhitespace(headerLine.charAt(i))) {
                i++;
            }
            ...
            headers.add(headerLine.substring(start, i));
            starts.add(start);
        }

        // Locate desired field
        int fieldIndex = -1;
        for (int j = 0; j < headers.size(); j++) {
            if (headers.get(j).equalsIgnoreCase(fieldName)) {
                fieldIndex = j;
                break;
            }
        }
        ...
        int start = starts.get(fieldIndex);
        int end = (fieldIndex + 1 < starts.size()) ? starts.get(fieldIndex + 1) : dataLine.length();

        if (start >= dataLine.length()) {
            return null;
        }

        end = Math.min(end, dataLine.length());

        String value = dataLine.substring(start, end).trim();

		if (!numericValue.matcher(value).matches() && isNumericField) {
			return null;
		}
