...
//				System.out.println("Incorrect property:"+PVLC+"\t"+badProp+"\t"+er.property_name);
	...
	private static String adjustPropertyValue(ExperimentalRecord er,String propertyValue) {
		...
		String[] badSolvents = { "heptane","hexene", "hexan", "ether", "ethyl", "benzene", "naoh", "hcl", "chloroform",
				...
				"bromoethane", "tetrachloroethane","propan-2-ol","thf","formamide","BuOH"};		
...
//			if(propertyValue==null) return null;
			...
			//*******************************************************************************************
			/*Following block doesnt help- only adds 3 records
			boolean hasNameInPV=false;
			if(er.chemical_name!=null && er.chemical_name.contains(solvent)) {
				hasNameInPV=true;
				System.out.println("has solvent in name:"+solvent+"\t"+er.chemical_name+"\t"+propertyValue);
//				System.out.println(er.chemical_name+"\t"+solvent+"\t"+propertyValue);
			}
			if(hasNameInPV) {			
				continue;
			}
			*/
			...
			for (String water:waterSynonyms) {
				...
				if (propertyValue.toLowerCase().contains(water) && !solvent.contains(water)) { // Handles "ammonia water" and similar
					...
					foundWater = true;
					propertyValue=getWaterSolubilityString(propertyValue, er,water);
					break;
				}
			}

			//				System.out.println(pv+"\t"+solvent+"\t"+foundWater);


			// If no water synonyms found, mark record as bad
			if (!foundWater) {
				er.keep = false;
//				System.out.println("No water:"+solvent+"\t"+er.chemical_name);
				er.reason = "Non-aqueous solubility";
				break;
			}
			
		
		}//end loop over bad solvents
	...
	static String getWaterSolubilityString(String propertyValue,ExperimentalRecord er,String water) {
	...
	public static boolean getToxicity(ExperimentalRecord er,ToxicityRecord tr) {
		...
		if (strippedReportedDose.contains("H")) {
			...
			try {
				...
				System.out.println(tr.NormalizedDose+"\tError normalizing");
//				ex.printStackTrace();
			}			
		} else if (strippedReportedDose.contains("M")) {
			String strM=strippedReportedDose.substring(strippedReportedDose.lastIndexOf("/")+1,strippedReportedDose.length()-1);
			try {
				double h=Double.parseDouble(strM)/60.0;
				//use haber's rule that C*t=k (see DOI: 10.1093/toxsci/kfg213 that shows this might not be great approx)	
				if (h!=4.0) {
					er.property_value_point_estimate_original*=h/4.0;
					er.note="Duration: "+TextUtilities.formatDouble(h)+" H, adjusted to 4 H using Haber's law (conc*time=constant)";
